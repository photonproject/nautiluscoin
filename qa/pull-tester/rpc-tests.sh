...
export NAUTILUSCOINCLI=${BUILDDIR}/qa/pull-tester/run-nautiluscoin-cli
export NAUTILUSCOIND=${REAL_NAUTILUSCOIND}
...
if [ "x${ENABLE_NAUTILUSCOIND}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and nautiluscoind must all be enabled"
fi
