...
*Setup instructions for a gitian build of Nautiluscoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Nautiluscoin
...
- [Building Nautiluscoin](#building-nautiluscoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-nautiluscoin-host/README.md).
...
Clone the git repositories for nautiluscoin and gitian and then checkout the nautiluscoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the nautiluscoin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building Nautiluscoin
----------------

To build Nautiluscoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the nautiluscoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/nautiluscoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/nautiluscoin/nautiluscoin
...
For example:
```bash
URL=https://github.com/laanwj/nautiluscoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit nautiluscoin=${COMMIT} --url nautiluscoin=${URL} ../nautiluscoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit nautiluscoin=${COMMIT} --url nautiluscoin=${URL} ../nautiluscoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit nautiluscoin=${COMMIT} --url nautiluscoin=${URL} ../nautiluscoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/nautiluscoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/nautiluscoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/nautiluscoin-build.assert
```
...
[nautiluscoin/gitian.sigs](https://github.com/nautiluscoin/gitian.sigs/) repository, or if that's not possible create a pull
