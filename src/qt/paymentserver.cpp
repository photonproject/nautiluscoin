...
const char* BIP71_MIMETYPE_PAYMENT = "application/nautiluscoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/nautiluscoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/nautiluscoin-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the nautiluscoin: URI contains a payment request, we are not able to detect the
        ...
        if (arg.startsWith(NAUTILUSCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // nautiluscoin: URI
        {
            ...
            if (GUIUtil::parseNautiluscoinURI(arg, &r) && !r.address.isEmpty())
            {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(NAUTILUSCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // nautiluscoin: URI
    {
        ...
        else // normal URI
        {
            ...
            {
                CNautiluscoinAddress address(recipient.address.toStdString());
                if (!address.IsValid()) {
                ...
                emit message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Nautiluscoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
