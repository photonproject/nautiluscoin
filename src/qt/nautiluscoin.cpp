...
public:
    ...
    /// Get window identifier of QMainWindow (NautiluscoinGUI)
...
void NautiluscoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new NautiluscoinGUI(networkStyle, 0);
...
void NautiluscoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
...
void NautiluscoinApplication::initializeResult(int retval)
{
    ...
    if(retval)
    {
        ...
        if(pwalletMain)
        {
            ...
            window->addWallet(NautiluscoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(NautiluscoinGUI::DEFAULT_WALLET);
...
WId NautiluscoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(nautiluscoin_locale);
    ...
    {
        QMessageBox::critical(0, QObject::tr("Nautiluscoin Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("Nautiluscoin Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    try
    {
        ...
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("Nautiluscoin Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
