...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("NautiluscoinMiner : ProcessNewBlock, block not accepted");
...
void static NautiluscoinMiner(CWallet *pwallet)
{
    ...
    try {
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in NautiluscoinMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running NautiluscoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
