...
public:
    ...
    CNautiluscoinAddress(const CTxDestination &dest) { Set(dest); }
    CNautiluscoinAddress(const std::string& strAddress) { SetString(strAddress); }
    CNautiluscoinAddress(const char* pszAddress) { SetString(pszAddress); }
...
public:
    ...
    CNautiluscoinSecret(const CKey& vchSecret) { SetKey(vchSecret); }
    CNautiluscoinSecret() {}
};
